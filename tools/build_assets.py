...
def load_icons(root: Path) -> list[tuple[str, int, int, list[int]]]:
    """Load the firmware icon catalog synced from g2ui (`assets-src/icons/library.json`)."""
    data = json.loads((root / "assets-src" / "icons" / "library.json").read_text(encoding="utf-8"))
    icons: list[tuple[str, int, int, list[int]]] = []
    used_names: set[str] = set()
    for item in data:
        icon_id = item["id"]
        name = "gui_icon_" + re.sub(r"[^A-Za-z0-9_]", "_", icon_id)
        if name[0].isdigit() or name == "gui_icon_":
            name = "gui_icon_id_" + name.removeprefix("gui_icon_")
        suffix = 2
        unique = name
        ...
        icons.append((unique, int(item["width"]), int(item["height"]), [int(v) for v in item["rows"]]))
    return icons


ICONS: list[tuple[str, int, int, list[int]]] = load_icons(Path(__file__).resolve().parents[1])
...
def main() -> None:
    ...
    print(f"generated {len(faces)} font faces and {len(ICONS)} icons")
